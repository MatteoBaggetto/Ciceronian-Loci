...
namespace Microsoft.MixedReality.Toolkit.Inspectors
{
    ...
    public class ButtonConfigHelperInspector : UnityEditor.Editor
    {
        ...
        private SerializedProperty seeItSayItLabelDirection;
        ...
        private void OnEnable()
        {
            ...
            seeItSayItLabelDirection = serializedObject.FindProperty("seeItSayItLabelDirection");
                    ...
                    if (labelFoldout)
                    {
                        ...
                        if (seeItSayItLabelProp.objectReferenceValue != null)
                        {
                            ...
                            if (seeItSayItLabel.activeSelf)
                            {
                                ...
                                if(seeItSayItLabelDirection != null)
                                {
                                    
                                    EditorGUILayout.PropertyField(seeItSayItLabelDirection);
                                    
                                    SeeItSayLabelDirectionEnum seeItSayLabelDirectionEnum = (SeeItSayLabelDirectionEnum)seeItSayItLabelDirection.enumValueIndex;

                                    var seeItSayItLabelRectTransform = seeItSayItLabel.transform.GetChild(0).GetComponent<RectTransform>();
                                    BoxCollider cbCollider = cb.GetComponent<BoxCollider>();
                                    if (cbCollider != null)
                                    {
                                        var buttonSize = cbCollider.size;
                                        var localXPosition = seeItSayItLabelRectTransform.rect.width + buttonSize.x / 2 + 0.008f;
                                        var localYPosition = seeItSayItLabelRectTransform.rect.height + buttonSize.y / 2;

                                        switch (seeItSayLabelDirectionEnum)
                                        {
                                            case SeeItSayLabelDirectionEnum.Left:
                                                seeItSayItLabel.transform.localPosition = new Vector3(-localXPosition, 0f, seeItSayItLabel.transform.localPosition.z);
                                                break;
                                            case SeeItSayLabelDirectionEnum.Right:
                                                seeItSayItLabel.transform.localPosition = new Vector3(localXPosition, 0f, seeItSayItLabel.transform.localPosition.z);
                                                break;
                                            case SeeItSayLabelDirectionEnum.Up:
                                                seeItSayItLabel.transform.localPosition = new Vector3(0f, localYPosition, seeItSayItLabel.transform.localPosition.z);
                                                break;
                                            case SeeItSayLabelDirectionEnum.Down:
                                                seeItSayItLabel.transform.localPosition = new Vector3(0f, -localYPosition, seeItSayItLabel.transform.localPosition.z);
                                                break;
                                            default:
                                                break;
